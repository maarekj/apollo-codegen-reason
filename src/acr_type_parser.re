...
let raise_malformed_type = (type_) =>
  raise(MalformedType(type_, sprintf("The type \"%s\" is malformed", type_)));
...
module Lexer = {
  ...
  let create_lexer = (str) => {string: str, current_pos: 0};
  type token =
    | LeftBrace
    | RightBrace
    | Word(string)
  ...
  let get_token = (lexer) =>
    if (lexer.current_pos >= String.length(lexer.string)) {
      ...
      let char = lexer.string.[lexer.current_pos];
      if (char == '[') {
        LeftBrace
      } else if (char == ']') {
        RightBrace
        ...
        | None => EOS
        | Some(result) => Word(Js.Re.matches(result)[0])
        }
      }
    };
  let inc_lexer = (lexer) => {
    ...
    {...lexer, current_pos: min(lexer.current_pos + inc, String.length(lexer.string))}
  };
};
...
module Parser = {
  ...
  let parse = (str) => {
    ...
    let rec scalar_rule = (lexer) =>
      switch (get_token(lexer)) {
      | Word(str) => Some((inc_lexer(lexer), Scalar(str)))
      | _ => None
      }
    and array_rule = (lexer) =>
      switch (get_token(lexer)) {
      | LeftBrace =>
    ...
    let rec normalize_ast = (optional: bool, node: gqlTypeAux) : gqlType =>
      switch node {
      | Array(node) =>
