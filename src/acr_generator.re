          ...
          sprintf
            ...
            (
              Type_parser.to_reason_type
                (fun _ => sprintf "%s%s_%s" parent_name field.response_name f.response_name) f.type_
            )
        }
    );
...
let rec type_flatten_fields parent_name (field: field) =>
  ...
  | Some sub_fields =>
    List.concat [
      List.concat (List.map (type_flatten_fields (parent_name ^ field.response_name ^ "_")) sub_fields),
...
let rec convert_to_reason_fields parent_name (field: field) =>
  ...
  | Some sub_fields =>
    List.concat [
      List.concat (List.map (convert_to_reason_fields (parent_name ^ field.response_name ^ "_")) sub_fields),
  ...
  List.map (
    fun (variable: variable) =>
      sprintf
        "  \"%s\": %s"
        variable.name
        (Type_parser.to_js_converter field::(sprintf "vars.%s" variable.name) type_::variable.type_)
  );
